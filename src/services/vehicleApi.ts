...
export interface Vehicle {
  ...
  receipt_id?: string;
  otp?: string; // OTP generated by backend during check-in
  created_by?: string;
  updated_by?: string;
  deleted_by?: string;
  created_on?: string;
...
export class VehicleAPI {
  ...
  // Verify OTP for vehicle checkout
  static async verifyOTP(vehicleId: string, otp: string): Promise<boolean> {
    try {
      const response = await apiClient.post<{ is_verify: boolean; statusCode: number }>(`/vehicles/verify-otp-checkout`, {
        vehicle_id: vehicleId,
        otp: otp
      });
      // Return true if statusCode is 201 and is_verify is true
      // The response.data contains { is_verify: true, statusCode: 201 }
      return response.statusCode === 201 && response.data?.is_verify === true;
    } catch (error: any) {
      console.error('OTP verification error:', error);
      // If API returns error, return false
      return false;
    }
  }
